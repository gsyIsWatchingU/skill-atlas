...
已补齐项目级启用、冲突阻断、幂等预览与 manifest 回滚。
...
**2026-09-24 进度：已落地。** 桌面端已支持创建、编辑、排序业务流，按顺序 / 并行 / 按需
组合 Skill，并生成一个 `$workflow-*` 调用入口。入口只保存 Skill 引用和版本哈希，不复制子 Skill
逻辑；不同业务流可复用同一子 Skill。已支持选择项目并向 Codex / Claude Code / Cursor 目录逐项
建立 Junction，执行前展示计划，同名冲突默认阻断，执行后可按 manifest 回滚。
已支持无本机路径、无 Skill 正文的业务流 JSON 导出 / 导入，导入时核对依赖和版本。
已支持 Codex Plugin 原生目录导出：入口与依赖 Skill 以固定版本快照写入 `skills/`，并通过
Plugin 官方校验器。业务流可使用现有统一账号保存到云端、跨设备拉取；云端按账号隔离并保留版本记录。
当前仓库实现与测试已完成；线上云 API 需部署本版本后才会开放业务流接口。
